...
Function.prototype.myBind1 = function (ctx) {
  // saving the function being called for scope
  const fn = this;



  //bindArgs will include all arguments except the first;
  const bindArgs = Array.from(arguments).slice(1);


  // using an anonomous function we make an
  // array consisting of all the arguments
  return function () {
    const callArgs = Array.from(arguments);

    // we then apply the function to all of the arguments
